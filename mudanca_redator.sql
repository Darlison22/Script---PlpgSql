-- Dados do processo a alterar
select tojc.ds_orgao_julgador_colegiado turma_secesp,
       to_char(tjs.dt_sessao, 'dd - mon/yyyy') data_sessao,
       tp.nr_processo numero_processo,
       tps.id_sessao id_sessao,
       toj.ds_orgao_julgador orgao_redator,
       tul.ds_nome nome_redator,
       *
from pje_jt.tb_pauta_sessao tps
inner join pje_jt.tb_jt_sessao tjs on tjs.id_sessao = tps.id_sessao
inner join pje.tb_orgao_julgador toj on toj.id_orgao_julgador = tps.id_orgao_julgador_redator
inner join pje.tb_orgao_julgador_colgiado tojc on tojc.id_orgao_julgador_colegiado = tjs.id_orgao_julgador_colegiado
inner join pje.tb_usuario_login tul on tul.id_usuario = tps.id_magistrado_redator
inner join pje.tb_processo tp on tp.id_processo = tps.id_processo_trf
where tps.id_tipo_situacao_pauta is not null
  and tp.nr_processo in ('0000099-96.2024.5.07.0037',
